...
import com.dmss.burbankapp.data.model.HouseAppointmentResponseModel
import com.dmss.burbankapp.databinding.FragmentFavoriteChooseYourTimeCommentBinding
...
class FavoriteChooseYourTimeCommentFragment : Fragment() {
    var selectedDate: String? = null
    lateinit var toolBinding: HomelandToolBinding
    lateinit var binding: FragmentFavoriteChooseYourTimeCommentBinding
    ...
    lateinit var localPlaces: List<String>
    ...
    override fun onCreateView(
        ...
        binding = FragmentFavoriteChooseYourTimeCommentBinding.inflate(inflater, container, false)
    ...
    private fun addHouseAppointment() {
        ...
        if (binding.lastName.text.isNotEmpty()) {
            lastName = binding.lastName.text.toString().trim().capitalize().trim()
        }
        ...
        if (firstName.isNotEmpty()) {
            if (lastName.isNotEmpty()) {
                if (!TextUtils.isEmpty(phoneNumber)) {
                    if (PhoneNumberUtils.isGlobalPhoneNumber(phoneNumber)) {
                        if (!TextUtils.isEmpty(localPlace)) {
                            if (isChecked) {
                                ...
                                if (obj != null) {
                                    fetchEnquiry(comment,houseName,houseSize,firstName, lastName, phoneNumber,etEmail,localPlace,
                                            time,
                                        date)
                            ...
                            AppUtils.showCustomCenterToast(requireContext(), "Please select where would you like to live")
                        }
                    } else {
                        AppUtils.showCustomCenterToast(
                            requireContext(),
                    ...
                    AppUtils.showCustomCenterToast(requireContext(), "Please enter Phone number")
                }

            } else {
                AppUtils.showCustomCenterToast(requireContext(), "Please enter Last Name")
            }

        } else {
            AppUtils.showCustomCenterToast(requireContext(), "Please enter First Name")
        }
        ...
        displayHomeViewModel.getHouseAppointmentLiveData().observe(viewLifecycleOwner, Observer {
            when (it.status) {
                Status.SUCCESS -> {
                    ...
                    loadFragment(FavoriteRequestSuccessfulFragment(), bundle)
        ...
        userInfoModel.FirstName?.let {
            if (it.isNotEmpty()) {
                ...
                binding.firstName.background = ContextCompat.getDrawable(
                    binding.firstName.context,
                    R.drawable.rectangel_comment_background
                )
    ...
    private fun getWebView(){
        ...
        if (arguments != null) {
            ...
            suburb = arguments?.getString("suburb")
            houseName = arguments?.getString("houseName")
            houseSize = arguments?.getString("houseName")
        }
