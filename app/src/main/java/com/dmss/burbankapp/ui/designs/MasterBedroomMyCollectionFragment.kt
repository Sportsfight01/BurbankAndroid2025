...
import com.dmss.burbankapp.databinding.FragmentMasterBedroomMyCollectionBinding
...
class MasterBedroomMyCollectionFragment : Fragment(), MainActivity.OnBackPressedListener,
    BreadCrumbAdapter.BreadcrumbItemClickListener {
    private var FEATURE = "Bedroom At Front"
    lateinit var binding: FragmentMasterBedroomMyCollectionBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentMasterBedroomMyCollectionBinding.inflate(inflater, container, false)
    ...
    private fun setupObserver() {
        myHomeQuizViewModel.getNewHomeNextQuestionLiveData()
            .observe(viewLifecycleOwner, Observer {
                when (it.status) {
                    Status.SUCCESS -> {
                        bottomLayoutBinding.progress.visibility = View.GONE
                            ...
                            if (newHomeNextFeatureResponseModel.status) {
                                newHomeNextFeatureResponseModel.newHomesNextFeatures?.HouseCount?.let { it1 ->
                                    AppUtils.getDesignsTextBasedOnCount(
                                        it1
                                    ).also { bottomLayoutBinding.tvDesigns.text = it }
                                }
                                NEWHOMESHOUSECOUNT =
                    ...
                    Status.LOADING -> {
                        bottomLayoutBinding.progress.visibility = View.VISIBLE
                    }
                    Status.ERROR -> {
                        bottomLayoutBinding.progress.visibility = View.GONE
                    }
    ...
    private fun initViews() {
        ...
        println("AppConstants.breadCrumbMyCollection:: "+AppConstants.breadCrumbMyCollection)
...
//        toolBinding.tvTool.text = mainActivity.showToolHeadText()
        ...
        binding.imusthave.setOnClickListener {
            ...
            AppConstants.toolheaderHashMap[FEATURE] = " | Bedroom At Front"
            AppConstants.breadCrumbMyCollection[FEATURE] =
                BreadCrumbHashMapModel("Bedroom At Front", FEATURE)
